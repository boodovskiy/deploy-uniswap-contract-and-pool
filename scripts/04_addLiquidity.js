...
async function main() {     
    ...
    await usdtContract.connect(signer2).approve(POSITION_MANAGER_ADDRESS,  ethers.utils.parseEther('0.25'))
    await usdcContract.connect(signer2).approve(POSITION_MANAGER_ADDRESS,  ethers.utils.parseEther('0.25'))
    
    const poolContract = new Contract(USDT_USDC_500, artifacts.UniswapV3Pool.abi, provider)

    const poolData = await getPoolData(poolContract)

    const chainId = 11155111 // 31337 - hardhat, 11155111 - sepolia
    const UsdtToken = new Token(chainId, TETHER_ADDRESS, 18, 'USDT', 'Tether')
    const UsdcToken = new Token(chainId, TETHER_ADDRESS, 18, 'USDC', 'UsdCoin')
    ...
    const position = new Position({
        pool: pool,
        liquidity: ethers.utils.parseEther('0.25'),
    ...
    params = {
        ...
        amount0Min: 0,
        amount1Min: 0,
        recipient: signer2.address, // leave or change
